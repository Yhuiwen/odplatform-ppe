...
| RISK-008 | RTSP 不稳定 | High | Medium | P7-4 已实现 RTSP adapter、URI 脱敏、timeout 设置和连接失败状态；P7-6 已通过受控本地 MediaMTX 流验证 open/read/close；远端 RTSP、重连策略和断流恢复仍未验证 | OPEN |
...
| RISK-021 | Streamlit rerun/session behavior couples UI lifecycle to long-running inference | High | High | P7-3 已实现只读 dashboard query/service boundary 和 session-scoped read runtime；P7-5 已执行全部四个 Streamlit 页面和本地 server health/root check；P7-6 已将 monitoring loop 放入 service-owned background worker，页面只控制 start/status/stop，并完成真实浏览器 MP4 session。长期 session lifecycle 和异常恢复仍需验证 | OPEN |
| RISK-022 | USB Camera or RTSP source stalls, disconnects or returns stale frames | High | High | P7-4 已将 MP4/USB/RTSP 隐藏在 `VideoSource` 后，实现 observable state、timeout、credential redaction、connection failure 和 release cleanup；P7-5 已验证真实 USB open/read/close 和 injected disconnect；P7-6 已验证受控本地 RTSP open/read/close，但远端 RTSP、bounded reconnect 与 stale-frame detection 仍待验证 | OPEN |
...
| RISK-024 | Web, Console or TTS delivery failure blocks or corrupts the event main path | Medium | High | persistence-first；P7-3 已实现 Console/Web adapter failure isolation、ordered fan-out 和 idempotency by `event_id`；P7-5 已验证两个 adapter 的实际 delivery path；P7-6 已实现 TTS cooldown/failure isolation、通过注入式测试并完成 native Windows SAPI 调用，但长期 delivery 仍需验证 | OPEN |
...
| RISK-027 | Annotated demo rendering may publish incomplete or undecodable output | Medium | Medium | M-007 uses same-volume staging, decoded frame-count/dimension verification, SHA256 metadata, atomic directory publication and rollback；one 47-frame real MP4 passed；long-duration throughput, codec portability and visual annotation quality review remain open | MONITORING |
